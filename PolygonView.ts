...
export default class PolygonView extends Laya.Sprite {
    ...
    private circleFillColor: string | undefined;
    private circleLineColor: string | undefined;
    private circleLineWidth: number | undefined;
    /** 绘制配色 */
     ...
     * @param pnts   配置数据列表
     */
    constructor(private radius: number, pnts: Point[]) {
        super();
    ...
    /** 设置圆旋转角度 */
    setCircleRotation(rotation: number): PolygonView {
    ...
    /** 设置图表半径 */
    setRadius(radius: number): PolygonView {
        this.reset(this.drawData, radius);
        this.redraw(this.shapeFillShow, this.polygonLineShow);
        return this;
    }
    ...
    reset(pnts: Point[], radius?: number): void {
        ...
        this.shapePoints = this.getDrawPoints(pnts, rotation, 'label', this.radius);
    ...
    private onRedraw(showCircleFill: boolean = true, showPolygonLine: boolean = true): void {
        this.graphics.clear();
        if (showCircleFill) this.drawCircle(this.radius, this.circleFillColor, this.circleLineColor, this.circleLineWidth);
    ...
    private getDrawPoints(pnts: Point[], rotation: number, label: 'label' | 'text', radius?: number): number[] {
        ...
        pnts.forEach((pnt: Point, idx: number) => {
            ...
            let ofs = this.offset(rot);
            let r = radius || (this.radius * pnt.scale);
            let x = ofs.x * r;
            let y = ofs.y * r;
            lps.push(x, y);
            //
    ...
    /** 计算当前角度XY移动偏移量 */
    private offset(rot: number): Laya.Point {
